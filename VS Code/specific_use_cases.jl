...
using Pkg
Pkg.add(["GLM", "DataFrames", "MixedModels", "Optim", "MultivariateStats", "RDatasets", "Flux", "ProgressBars", "Plots", "DifferentialEquations"]);
...
# We will regress the reaction time against the number of days of 
# sleep deprivation (with an intercept)

fm = @formula(reaction ~ 1 + days)
model = fit(LinearModel, fm, data)

# The coefficients are obtained with 'coef' and their standard error 
# with 'stderror'
...
# If we wanted to incorporate the random effect of subjects, 
# we need to use mixed models.
...
# To obtain the variance of the fixed effects, 
# an unfortunately long notation is used
...
# The main syntax is: define an objective function that takes in a 
# vector of parameters x and spits out a number. 
# Use the 'optimize' function with an initial point and potential bounds,
...
# for all possible configurable options such as tolerance, 
# function calls and time limit
...
# Unfortunately, there is no way to guarantee global convergence if 
# our function is not convex.
# One good remedy is to do multiple starting points, 
# and select the minimum from all starts.
...
# If your function is univariate (over one variable), 
# you can converge using bounds without needing a starting point
...
# If you have more than one variable, and you wish to impose bounds, 
# you must specify a gradient.
# If you can't compute gradients explicitely, you can use automatic 
# differentiating to do it for you.
# Below is an example
...
# We will use a dataset that measures flowers in 4 ways (4 variables) and 
# labels them into three categories.
# We will use multiple component analysis tools to reduce the dimensionality 
# from 4 to 2
...
Pkg.add(["Flux", "ProgressBars"]);
using Flux, ProgressBars, Plots, Statistics

# This is the most prominent deep learning package in 
# Julia with great CUDA support.
...
# The model is very simply, it has 3 hidden units with tanh activation,
# a batchnorm and a softmax at the end.
...
# Here is where a lot of hyperparameters go, we'll keep it 
# simple and use Adam with a momentum parameter of 0.01
...
# This is perhaps the most successful and well-known package in Julia. 
# It is the gold-standard if you're numerically solving any type of 
# differential equation. 
# It works by setting up a differential equation problem, 
# and then using 'solve' to solve it.
...
# The dynamics function represents the right hand side of this 
# equation and takes in three arguments:
# - u the variable of interest (which may be a vector)
# - p the parameters of the model
# - t the timepoint
# We then set the initial conditions (here to 0.5) and the time range 
# to between 0 and 1
...
# There are many arguments solve can take such as which algorithm to use, 
# or which tolerance.

sol = solve(prob)

# In the multivariate case, the dynamics function 'f' would be slightly different.
# Below is the Lorenz attractor (3-dimensional). In this case, 
# we have an extra argument: du
# du is the vector of gradients that we will fill in. 
# We do this in place for memory reasons
