...
# You should see a tick mark next to the function, which means it has executed 
# without error. You should also see 'Hello world' printed in the terminal, 
...
# They allow us to use other people's work, specializing what we want our 
# code to do. Packages can either be from the standard library which comes 
# with your Julia installation, or they are installed from the internet.
# To load a package, run the following line:

using LinearAlgebra

# This is one of the standard library packages, so you don't need to do
# any installations.
...
# The second (and in my opinion easier) way to install a package is by
# using the terminal.
# Press the right bracket ']' button while you are in the terminal,
# and it should turn from green to blue.
# You are now in "package mode". From here, it is very easy to install, 
# update, or remove packages. Simply type 'add Benchmarktools' while in 
# package mode to install the package.
...
# Similarly to MATLAB, this model gives details about any function you 
# might wish to use. Try type 'svd' and press enter. 
# You should get some information about how the svd function works.

# Next, try enter package mode, and type '?' followed by enter. 
# This will give you an overview of all the functions you can run.
# For example, it shows that you can use 'status' to check all your 
# installed packages.

# The final mode is shell mode. Enter this mode by pressing the 
# semi-colon ';' button
# From here, you can write anything you would in the terminal 
# such as 'ls', 'cd' etc
...
# You will see that after executing each of these lines, 
# the value of the variable will be displayed on the right.
# To stop this from happening, put a semi-colon ';' after the line
a = 2;
b = "three";
c = [1];

# If you don't want to run each line individually, you can use 
# the 'begin' 'end' functionality
begin
    a = 3
    b = "one"
    c = [2]
...
# This is purely aesthetic, but you can use any unicode character 
# from https://docs.julialang.org/en/v1/manual/unicode-input/
# To use these, type the common name such as 'alpha' after a backslash, 
# and press [TAB] ('\alpha' + [TAB])
α = "[BACKSLASH]alpha[TAB]";
😎 = "[BACKSLASH]:sunglasses:[TAB]";
ℵ = "[BACKSLASH]aleph[TAB]";

# You can also use this technique to do sub/super scripts.
# To do this, type '\_1' or '\^2'. Note: you can't sub/super script anything
σ² = "[BACKSLASH]sigma[TAB] [BACKSLASH]^2[TAB]";
Xₜ = "X [BACKSLASH]_t[TAB]";
αᵅ = "[BACKSLASH]alpha[TAB] [BACKSLASH]^alpha[TAB]";

########## TYPES ##########

# While you don't have to worry/care about types if you wish, 
# it still helps in understanding, potentially speeding up, and 
# safe-guarding your code.
...
# Concrete types are what you get from 'typeof', and they are the most specific 
# description of your variable.
# Abstract types are more general, and can be used to check broader categories.
# For example, whether your variable is a Float64 or an Int64, they are
# both numbers.
...
# A very useful tool to compact things is the ternary operator which uses 
# the question mark '?'
# The way this operator works is as follows: you first provide a statement 
# which could be true or false
# Then you write the questoin mark, and then you write what follows if 
# the statement is true. After this, you write a colon ':', 
# and then comes what has to happen is the statement is false.
...
# Another very common sight in programming are loops. These let you execute 
# bits of code multiple times. I will cover for loops, but not while loops, 
# since I don't find them very useful (but they exist in Julia)
# If we want to iterate over 10 numbers, we first create the range
# of numbers as in MATLAB by 1:10

for i in 1:10
    print(i)
end

# Why did it print everything on one line? That's because most of the time 
# we actually want to use 'println' and not 'print'
...
# If you want to break the loop early, because you achieve what you wanted, 
...
# Similarly to the ternary operator '?', we can do loops in a single line 
# (same as Python's list comprehension)
# Note that here, the results will go into an array (which we will cover next)
a = [i for i in 1:10]

### EXERCISE 1

# Write a loop that counts from 1 to 100, printing each number on a separate 
# line. But, there's a twist:
...
# We have previously discussed primitive types, but we can do more interesting 
# stuff with them. For example, once you have a number, you may be interested 
# in forming a collection of numbers
# In Julia, there is a number of ways to store these numbers, the simplest 
# of them being the array
# Like in MATLAB (and unlike Python), 1D arrays are vectors and 2D arrays 
# are matrices
# Unlike MATLAB (and like Python), it is very easy to add onto these arrays
...
# Like with the range 1:10, we can loop over the contents of an array using 
# 'in' (or ∈ = '\in')
...
# You can use 'in' (or '∉' '\notin') to check if an array contains 
# (or does not contain) an item

2 ∈ a
3 ∉ a

# You can use the 'enumerate' function to keep track of the index as well 
# as the value
...
# If you want to add/remove numbers to the front of the array, use 
# 'pushfirst!' and 'popfirst!'

pushfirst!(a, 6)
popped_number = popfirst!(a)

# Like MATLAB (and unlike Python), Julia uses 1 indexing. 
# (Mathematical consistency, Readability and clarity, 
...
# If you want to initialize a Vector as being empty, 
# I recommend setting the type of the Vector as follows:
...
# To get the unique elements of an array, use 'unique'
...
# The third basic data structure is the dictionary. 
# This is implemented as a hash map
# Dictionaries are useful if you want to index your data structure arbitrarily.
# For example, to store the data for different subjects, 
# you may want to use a dictionary since you can use the subject IDs 
# to access the data
...
# You can use the functions 'keys' and 'values' to access 
# the set of keys and values of the dictionary.
...
# Write a piece of code that for the given dictionary, 
# creates a separate dictionary where the values in the arrays repeated. 
# For example:
...
# Julia is a funcional programming language. What that means is that 
# everything is done through functions.
# As we will see later, there are objects called 'structs' that have some 
# similarities to classes in python,
# but Julia is not an object oriented programming language.
# To define your own function, use the 'function' keyword. 
# Use 'return' to indicate what the function should return when called
...
# If you want to bullet-proof your code, assigning a type to the input 
# and outputs of your function makes sure that it takes in and spits out 
# things in the correct format. 
# We can assign a type to the inputs using the double colon notation '::' 
# followed by the type
# To assign a type to the output, use the double colon after the name of 
# the function
...
# docstrings: explanations about how your function works, 
# can be implemented using triple quotations """
# The format in Julia is the have the docstring right above your function, 
# and as first line, have the name of the function with some inputs with 
# four space before, followed by a one-line explanation of what the function 
# does in present tense.
...
# Now try and go into 'help mode' by pressing '?' in the terminal and then 
# typing the function's name sum_across_subjects, you should see that the 
# docstring is now displayed.

# One neat feature of julia is that you can broadcast your function over arrays.
# What that means is that we can have a function (e.g. sum) and apply it 
# to every element of an array.
# To do this, write a dot/period after the function and before the parentheses
# 'sum.(array)'
# Lets collect all the values of our data dict, and broadcast the 
# sum operator over it.
...
# This code returns an error because it does not know how to multiply 
# two row vectors together
# The line below broadcasts the multiply operator over each element of 
# the vector.

a .* b

# Some functions in Julia have an exclamation mark in them 
# (like push! and pop! that we used earlier).
# The exclamation mark is a convention in Julia to indicate that these 
# functions work in place. What this means is that these functions don't 
# return anything: they modify their arguments.
...
# Similar to Python's lambda functions, Julia also has 'anynonymous functions'
# (aka one line functions)
...
# These anonymous functions can be useful in a number of cases. 
# The most immediate is to find the position of an element in an array. 
# This uses the 'findfirst' or 'findall' functions
# These functions take a 'check' function as first argument to tell them what 
# to look for. This check is a statement that is applied to all elements, 
# and those that satisfy it are returned
...
# If we only want the values, but not the indices, 
# we can use the broadcasting operator.
# For example, in the following array, 
# imagine we only want the lists with length greater than 2.

a = [[1, 2, 3], [3, 3, 5, 6, 2], [2], [4, 2], [5, 2], [2], [2, 4, 6, 7]]

# We fist generate an array with a 1 if the element satisfied our demand, 
# and 0 otherwise

b = length.(a) .> 2

# Then we can index our original array with this 'BitVector' 
